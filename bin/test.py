...
try:
    ...
    egg_dir = SPLUNK_HOME + '/etc/apps/compuware_apm_syn/bin/'
...
SCHEME = '''<scheme>
    ...
    <endpoint>
        <args>
            ...
            <arg name="site_id">
                <title>Site IDs</title>
                <description>Site IDs. Location where Monitor IDs where ran from (use commas to separate multiple locations or leave blank for all).</description>
            ...
            <arg name="data_designator">
                <title>Data Designator</title>
                <description>Specifies data to collect transaction summary, page, host, etc.  Defaults to ALL</description>
...
def run():
    ...
    #config = get_config()
    config = {'name': 'x',
              'monitor_id': '1181422,1181420,17016439',
              'username': 'x243',
              'password': 'cheve11e',
              'interval': '900',
              'data_designator': 'TIME'
              }
    ...
    logging.debug('CompuWare API Prams OpenDataFeed2: monitorIds=%s' +
                  'monitorClassDesignator=%s startTime=%s endTime=%s',
                  monitor_id,
                  monitor_class,
    ...
    try:
        print site_id
        print monitor_id
        gpn_data.OpenDataFeed2(monitorIds=monitor_id,#{'int': monitor_id},
                                 siteIds=None,
